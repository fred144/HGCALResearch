#define My2p9to3p0sum_cxx
// The class definition in 2p9to3p0sum.h has been generated automatically
...
//
// root> T->Process("2p9to3p0sum.C")
// root> T->Process("2p9to3p0sum.C","some options")
// root> T->Process("2p9to3p0sum.C+")
//


#include "My2p9to3p0sum.h"
#include <TH2.h>
#include <TStyle.h>
#include <TH1.h>
#include <iostream>

void My2p9to3p0sum::Begin(TTree * /*tree*/)
{
...
void My2p9to3p0sum::SlaveBegin(TTree * /*tree*/)
{
   ...
   TString option = GetOption();
   h1 = new TH1F("h1", "Reconstructed Energy 1 TeV (eta = 2.9 - 3.0)", (1100-800)/5 , 800, 1100);
   GetOutputList() -> Add (h1);
   h1->SetXTitle("Energy (Gev)");
   h1->SetYTitle("Number of Entries");
}

Bool_t My2p9to3p0sum::Process(Long64_t entry)
{
...
void My2p9to3p0sum::SlaveTerminate()
{
...
void My2p9to3p0sum::Terminate()
{
   ...
   TFile file_out("histo2930eres.root","RECREATE");
   h1->Write();
   file_out.ls();
   file_out.Close();
